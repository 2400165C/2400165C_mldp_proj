...
if model:
    ...
    with col2:
        ...
        st.caption("(-1 = paid duly, 0 = revolving, 1-8 = delayed)")
        pay_0 = st.slider("Status in Sep, 2005 (PAY_0)", -1, 8, 0)
        pay_2 = st.slider("Status in Aug, 2005 (PAY_2)", -1, 8, 0)
        pay_3 = st.slider("Status in Jul, 2005 (PAY_3)", -1, 8, 0)
        pay_4 = st.slider("Status in Jun, 2005 (PAY_4)", -1, 8, 0)
        pay_5 = st.slider("Status in May, 2005 (PAY_5)", -1, 8, 0)
        pay_6 = st.slider("Status in Apr, 2005 (PAY_6)", -1, 8, 0)
    ...
    if st.button("Analyze Customer Risk", use_container_width=True, type="primary"):
        ...
        if prediction == 1:
            ...
            st.metric(label="Default Probability", value=f"{prediction_proba[1]*100:.2f}%")
        else:
